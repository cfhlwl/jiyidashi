...
from app.services.annual_summary_service import (
    AnnualSummaryError,
    summarize_year,
)
...
class _MutatingProvider:
    ...
    async def infer(self, request: AIInferenceRequest) -> AIProviderResult:
        ...
        return AIProviderResult(
            ...
            provider="annual-mutating-test",
            model="fixture",
            provider_request_id="annual-mutation",
        )

    async def infer_image(self, request):
        del request
        raise AssertionError("Annual Summary must not call image inference")
...
class _FailingProvider:
    ...
    async def infer(self, request: AIInferenceRequest) -> AIProviderResult:
        self.requests.append(request)
        raise AIProviderError("ANNUAL_SUMMARY_PROVIDER_FIXTURE_FAILED")
...
async def test_persisted_timezone_computes_exact_local_year_boundary():
    with SessionLocal() as db:
        ...
        inside, _ = _memory(
            ...
            owner,
            content="本地新年第一分钟的事实",
            occurred_at=datetime(2025, 12, 31, 16, 0, tzinfo=UTC),
        )
        _memory(
            db,
            owner,
            content="上一年最后一分钟",
            occurred_at=datetime(2025, 12, 31, 15, 59, tzinfo=UTC),
        )
        _memory(
            db,
            owner,
            content="下一年第一分钟",
            occurred_at=datetime(2026, 12, 31, 16, 0, tzinfo=UTC),
        )
        gateway, provider = _gateway(
            '{"summary":"2026 年发生了一件可信事实","citations":["Y1"]}'
        )
        ...
        assert "本地新年第一分钟的事实" in prompt
        assert "上一年最后一分钟" not in prompt
        assert "下一年第一分钟" not in prompt


@pytest.mark.asyncio
async def test_current_year_defaults_from_server_reference_and_timezone():
    reference = datetime(2025, 12, 31, 16, 30, tzinfo=UTC)
    with SessionLocal() as db:
        owner = _owner(db, "current-year", timezone="Asia/Shanghai")
        inside, _ = _memory(
            db,
            owner,
            content="本地已经进入 2026 年",
            occurred_at=datetime(2025, 12, 31, 16, 10, tzinfo=UTC),
        )
        gateway, _ = _gateway(
            '{"summary":"本地 2026 年已经开始","citations":["Y1"]}'
        )
...
@pytest.mark.asyncio
async def test_december_31_to_january_1_boundary_is_exact():
    with SessionLocal() as db:
        owner = _owner(db, "year-boundary", timezone="UTC")
        inside, _ = _memory(
            db,
            owner,
            content="2026 年最后一分钟",
        ...
        gateway, _ = _gateway(
            '{"summary":"2026 年最后一分钟","citations":["Y1"]}'
        )
...
@pytest.mark.asyncio
async def test_leap_year_february_29_remains_inside_correct_year():
    with SessionLocal() as db:
        ...
        _memory(
            ...
            owner,
            content="2029 年第一分钟",
            occurred_at=datetime(2029, 1, 1, 0, 0, tzinfo=UTC),
        )
        gateway, _ = _gateway(
            '{"summary":"2028 年包含闰日事实","citations":["Y1"]}'
        )
...
@pytest.mark.asyncio
@pytest.mark.parametrize("target_year", ["2026-01", "26", "abcd", "9999", 2026])
async def test_invalid_target_year_is_rejected_before_provider(target_year):
    with SessionLocal() as db:
        owner = _owner(db, "invalid-year")
        gateway, provider = _gateway(
            '{"summary":"不应生成","citations":["Y1"]}'
        )

        with pytest.raises(AnnualSummaryError, match="INVALID_TARGET_YEAR"):
...
@pytest.mark.asyncio
async def test_cross_year_visit_overlap_and_finalized_gate():
    with SessionLocal() as db:
        ...
        visit, place = _visit(
            ...
            owner,
            place_name="跨年书店",
            arrived_at=datetime(2025, 12, 31, 23, 50, tzinfo=UTC),
            left_at=datetime(2026, 1, 1, 0, 20, tzinfo=UTC),
            finalized=True,
        )
        _visit(
            db,
        ...
        gateway, provider = _gateway(
            '{"summary":"2026 年开始时仍在书店","citations":["Y1"]}'
        )
        ...
        assert str(visit.id) not in prompt
        assert str(place.id) not in prompt
...
async def test_raw_visit_cap_is_incomplete_and_provider_not_called():
    with SessionLocal() as db:
        ...
        assert result.status == AnnualSummaryStatus.SUMMARY_INCOMPLETE
        assert result.incomplete_code == "YEAR_VISIT_SCAN_LIMIT_REACHED"
        assert isinstance(provider, DeterministicAIProvider)
        assert provider.requests == []
...
async def test_provider_slot_cap_is_incomplete():
    with SessionLocal() as db:
        ...
        assert result.status == AnnualSummaryStatus.SUMMARY_INCOMPLETE
        assert result.incomplete_code == "YEAR_AUTHORITATIVE_SLOT_LIMIT_REACHED"
        assert isinstance(provider, DeterministicAIProvider)
        assert provider.requests == []
...
async def test_provider_char_cap_is_incomplete():
    with SessionLocal() as db:
        ...
        assert result.status == AnnualSummaryStatus.SUMMARY_INCOMPLETE
        assert result.incomplete_code == "YEAR_EVIDENCE_CHAR_LIMIT_REACHED"
        assert isinstance(provider, DeterministicAIProvider)
        assert provider.requests == []


@pytest.mark.asyncio
async def test_single_memory_text_cap_is_incomplete():
    with SessionLocal() as db:
        owner = _owner(db, "single-char-cap", timezone="UTC")
        _memory(
            db,
            owner,
            content="年" * 1201,
            occurred_at=datetime(2026, 6, 1, 1, 0, tzinfo=UTC),
        )
        gateway, provider = _gateway(
            '{"summary":"不应生成","citations":["Y1"]}'
        )
        ...
        assert result.status == AnnualSummaryStatus.SUMMARY_INCOMPLETE
        assert result.incomplete_code == "YEAR_MEMORY_TEXT_LIMIT_REACHED"
        assert isinstance(provider, DeterministicAIProvider)
        assert provider.requests == []
...
async def test_prompt_uses_opaque_slots_and_injection_stays_data():
    with SessionLocal() as db:
        ...
        assert '"slot":"Y1"' in request.input_text
        assert '"local_year"' not in request.input_text
        assert '"timezone"' not in request.input_text
        assert '"output_contract"' not in request.input_text
...
@pytest.mark.parametrize(
    ...
    [
        (
            '{"summary":"x","citations":["Y1"],"complete":true}',
        ...
        (
            '{"summary":"x","citations":["Y1","Y1"]}',
            AnnualSummaryStatus.MALFORMED_PROVIDER_OUTPUT,
        ),
        (
            '{"summary":"x","citations":["Y9"]}',
            AnnualSummaryStatus.INVALID_CITATION,
        ),
    ],
)
...
async def test_provider_failure_is_typed():
    with SessionLocal() as db:
        ...
        assert result.status == AnnualSummaryStatus.PROVIDER_FAILED
        assert result.provider_error_code == "ANNUAL_SUMMARY_PROVIDER_FIXTURE_FAILED"
...
async def test_excluded_memory_gaining_evidence_invalidates_result():
    with SessionLocal() as db:
        ...
        assert "已有可信年事实" in prompt
        assert "生成期间才获得证据的年事实" not in prompt
...
@pytest.mark.asyncio
async def test_new_year_memory_during_provider_io_invalidates_inventory():
    with SessionLocal() as db:
...
@pytest.mark.asyncio
async def test_timezone_change_during_provider_io_invalidates_year_bounds():
    with SessionLocal() as db:
...
@pytest.mark.asyncio
async def test_persisted_invalid_timezone_drift_still_invalidates_result():
    with SessionLocal() as db:
        owner = _owner(db, "invalid-timezone-race", timezone="Invalid/Zone-A")
        _memory(
            db,
            owner,
            content="fallback UTC 下的年度事实",
            occurred_at=datetime(2026, 6, 1, 1, 0, tzinfo=UTC),
        )
        ...
        def mutate() -> None:
            with SessionLocal() as other:
                user = other.get(User, owner)
                assert user is not None
                user.timezone = "Invalid/Zone-B"
                other.commit()

        provider = _MutatingProvider(
            mutate,
            '{"summary":"旧时区快照","citations":["Y1"]}',
        )
        result = await summarize_year(
            db,
            user_id=owner,
...
@pytest.mark.asyncio
async def test_visit_provenance_drift_invalidates_visit_snapshot():
    with SessionLocal() as db:
        owner = _owner(db, "visit-provenance-race", timezone="UTC")
        visit, _ = _visit(
            db,
            owner,
            place_name="可信地点",
            arrived_at=datetime(2026, 7, 8, 1, 0, tzinfo=UTC),
        )

        def mutate() -> None:
            with SessionLocal() as other:
                current = other.get(Visit, visit.id)
                assert current is not None
                current.source_fingerprint = "b" * 64
                other.commit()

        provider = _MutatingProvider(
            mutate,
            '{"summary":"旧地点证据","citations":["Y1"]}',
        )
        result = await summarize_year(
            db,
            user_id=owner,
...
@pytest.mark.asyncio
async def test_caller_dirty_state_cannot_enter_year_snapshot():
    with SessionLocal() as db:
        ...
        assert isinstance(provider, DeterministicAIProvider)
        prompt = provider.requests[0].input_text
        assert "已提交的九年事实" in prompt
        assert "未提交的伪造年事实" not in prompt
        assert set(db.dirty) == before_dirty


@pytest.mark.asyncio
async def test_annual_summary_is_read_only():
    with SessionLocal() as db:
        ...
        before_reminders = int(
            db.scalar(select(func.count()).select_from(Reminder)) or 0
        )
        gateway, _ = _gateway(
            '{"summary":"九月只读测试","citations":["Y1"]}'
        )
        ...
        assert (
            int(db.scalar(select(func.count()).select_from(Reminder)) or 0)
            == before_reminders
        )


def test_s3_017_adds_no_public_annual_summary_endpoint():
    paths = {
        path
