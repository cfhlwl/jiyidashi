"""Trust-preserving complete-month summarization for S3-016."""
...
from app.monthly_summary_models import (
    MonthlySummaryCitation,
    MonthlySummaryResult,
    MonthlySummarySlotKind,
...
_MONTHLY_SUMMARY_PURPOSE = "memory.monthly_summary"
_TARGET_MONTH_RE = re.compile(r"^(?P<year>\d{4})-(?P<month>\d{2})$")
_MAX_MONTH_MEMORY_ROWS = 256
_MAX_MONTH_VISIT_ROWS = 128
_MAX_PROVIDER_SLOTS = 96
...
_SYSTEM_INSTRUCTION = """Summarize one complete server-supplied local calendar month.
...
def _read_session(db: Session) -> Session:
    # [人工注释][S3-016] Monthly Summary 只看 committed authoritative state；
    # caller pending/dirty identity map 不能改变 timezone、月界线或 provider context。
    return Session(
...
def _parse_target_month(value: str) -> tuple[int, int]:
    match = _TARGET_MONTH_RE.fullmatch(value)
    if match is None:
        raise MonthlySummaryError("INVALID_TARGET_MONTH")
...
def _load_month_context(
    ...
    *,
    ...
    with _read_session(caller_db) as db:
        ...
        if user is None:
            raise MonthlySummaryError("MONTHLY_SUMMARY_USER_NOT_FOUND")
    ...
    if target_month is None:
        ...
        year, month = local_reference.year, local_reference.month
    else:
    ...
    with _read_session(caller_db) as db:
        memories = list(
            db.scalars(
                ...
                .limit(_MAX_MONTH_MEMORY_ROWS + 1)
            )
        )
        if len(memories) > _MAX_MONTH_MEMORY_ROWS:
            return (), (), "MONTH_MEMORY_SCAN_LIMIT_REACHED"
        ...
        visit_rows = db.execute(
            ...
            .limit(_MAX_MONTH_VISIT_ROWS + 1)
        ).all()
        if len(visit_rows) > _MAX_MONTH_VISIT_ROWS:
            return (), (), "MONTH_VISIT_SCAN_LIMIT_REACHED"
    ...
    *,
    ...
    with _read_session(caller_db) as db:
        ...
        if len(content) > _MAX_MEMORY_TEXT_CHARS:
            raise MonthlySummaryError("MONTH_MEMORY_TEXT_LIMIT_REACHED")
...
def _build_slots(
    ...
    if len(pending) > _MAX_PROVIDER_SLOTS:
        return (), "MONTH_AUTHORITATIVE_SLOT_LIMIT_REACHED"
    total_chars = sum(len(row[6]) for row in pending)
    if total_chars > _MAX_TOTAL_EVIDENCE_CHARS:
        return (), "MONTH_EVIDENCE_CHAR_LIMIT_REACHED"
...
def _serialize_prompt(snapshot: _MonthSnapshot) -> str:
    evidence = []
    ...
    return json.dumps(
        {
            "local_month": snapshot.context.target_month,
...
def _parse_provider_output(
    ...
    *,
...
) -> tuple[MonthlySummaryStatus, str | None, tuple[str, ...]]:
    try:
        payload = json.loads(output_text)
    except (TypeError, json.JSONDecodeError):
        return MonthlySummaryStatus.MALFORMED_PROVIDER_OUTPUT, None, ()
    ...
    if set(normalized) != set(expected_slots) or len(normalized) != len(expected_slots):
        return MonthlySummaryStatus.INVALID_CITATION, None, ()
    return MonthlySummaryStatus.MONTHLY_SUMMARY_READY, summary.strip(), normalized
...
def _context_unchanged(
    ...
    *,
    user_id: UUID,
    original: _MonthContext,
) -> bool:
    try:
        current = _load_month_context(
            caller_db,
            user_id=user_id,
...
def _inventory_unchanged(
    ...
    *,
    ...
    if not _context_unchanged(
        ...
        return False
    memories, visits, incomplete = _scan_raw_month(
        caller_db,
        user_id=user_id,
...
def _citations(
    slots: Sequence[_PromptSlot],
    cited: Sequence[str],
) -> tuple[MonthlySummaryCitation, ...]:
    by_slot = {item.slot: item for item in slots}
    result: list[MonthlySummaryCitation] = []
...
async def summarize_month(
    ...
    *,
    ...
    """Summarize one complete bounded authoritative local calendar month."""

    reference = _as_utc(reference_utc or datetime.now(UTC))
    context = _load_month_context(
        db,
        user_id=user_id,
        target_month=target_month,
        reference_utc=reference,
    )
    raw_memories, raw_visits, incomplete = _scan_raw_month(
        db,
        user_id=user_id,
        context=context,
    )
    if incomplete is not None:
        return _empty_result(
            MonthlySummaryStatus.SUMMARY_INCOMPLETE,
            context=context,
    ...
    # [人工注释][S3-016] provider 前冻结“完整月”：raw inventory、所有 raw Memory
    ...
    if not _inventory_unchanged(db, user_id=user_id, snapshot=snapshot):
        return _empty_result(
            MonthlySummaryStatus.DATA_CHANGED_DURING_GENERATION,
            context=context,
        )
    ...
    if not slots:
        return _empty_result(
            MonthlySummaryStatus.NO_SUMMARIZABLE_EVIDENCE,
            context=context,
        )
    ...
    try:
        inference = await ai_gateway.infer(
            AIInferenceRequest(
                purpose=_MONTHLY_SUMMARY_PURPOSE,
    ...
    # Provider 看过整个 visible month context；citations 不是“模型用了哪些 context”的证明。
    ...
    if status != MonthlySummaryStatus.MONTHLY_SUMMARY_READY or summary is None:
        result = _empty_result(status, context=context)
        return replace(result, ai_provenance=inference.provenance)

    return MonthlySummaryResult(
        status=MonthlySummaryStatus.MONTHLY_SUMMARY_READY,
        target_month=context.target_month,
        timezone=context.timezone,
        summary=summary,
