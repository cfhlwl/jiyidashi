...
> 最后更新：2026-09-16  
> 当前阶段：Stage 1「记得住」第三批并行开发进行中；A：Backend Media 第一轮预审修复已完成并待第二轮审查；Stage 2 未开始  
> 当前开发基线：`main=b09a3981e12904fe488a623b1a5e1ee1314af6a8`  
> 当前 PR：#7 `feat/stage1-media-pipeline`（Draft，第一轮 3P1+2P2 已修，待第二轮审查）
...
| S1-M3 | Stage 1 第三批“多媒体记录 + 离线 + 数据控制” | 🔵 | A：Backend Media 第一轮修复后恢复 🟠 待第二轮审查；其余第三批工作线仍按独立分支推进 |
...
| S1-005 | 图片记忆录入 | 🔵 | A 线后端媒体/Evidence 第一轮修复已通过 53/53，待第二轮审查；主动拍照/选图客户端仍由 C 线实现 |
| S1-006 | COS / OSS 对象存储直传 | 🟠 | PR #7 第一轮 3P1+2P2 已修：commit-safe staging 清理、生产 HTTPS、真实图片签名、create_schema 显式媒体模型、公共 ETag 移除；待第二轮审查/合并 |
...
| --- | --- | --- | --- | --- | --- |
| A：Backend Media | `S1-006` + `S1-005` 后端媒体/Evidence 基础 | `feat/stage1-media-pipeline` / PR #7 | 私有对象存储、临时签名上传/下载、媒体元数据、图片 Evidence、Backend tests、API 文档 | 媒体协议已冻结为 staging→final；第一轮 3P1+2P2 已窄修；未引入 OCR/Vision/ASR/Stage 2 | 🟠 待第二轮审查 |
...
A 线当前自动验收：生产代码 HEAD `13cd13f3`；Ruff PASS；SQLite migration PASS；PostgreSQL migration PASS；`alembic check` PASS；PostgreSQL ObjectLocation invariants PASS；pytest **53/53 PASS**。PR #7 继续保持 Draft，等待第二轮人工审查，不自动合并。
...
| ID | 优先级 | 问题 | 状态 | 当前结果 |
| --- | --- | --- | --- | --- |
| S1-PR7-FIX-001 | P1 | READY commit 前删除 staging，commit 失败后无法恢复 | 🟠 | 已改为 READY commit 成功后才 best-effort 清 staging；commit-failure 回归确认 DB=PENDING、staging 保留、final 可存在且重试可恢复 |
| S1-PR7-FIX-002 | P1 | production 自定义对象存储 endpoint 可使用 HTTP | 🟠 | production/prod + S3 + custom endpoint 强制 HTTPS；development/test 仍允许本地 HTTP |
| S1-PR7-FIX-003 | P1 | READY 只验证 size/MIME，不能证明真实图片 | 🟠 | staging 与 final 均做小范围文件头校验；JPEG/PNG/WebP magic bytes、HEIC/HEIF `ftyp` brand；非法对象返回 `MEDIA_IMAGE_INVALID` 且保持 PENDING |
| S1-PR7-FIX-004 | P2 | `create_schema()` 依赖偶然 import 顺序加载媒体模型 | 🟠 | 已显式 import `media_models`，与 Alembic drift gate 语义对齐 |
| S1-PR7-FIX-005 | P2 | 公共 `MediaRead` 暴露内部 `storage_etag` | 🟠 | DB 内部 ETag 保留；公开 Media API 已移除该字段，客户端继续只依赖 opaque `media_id` |

> 第一轮修复生产代码范围：`c553b121..13cd13f3`。上述 5 项已在同一生产 HEAD 通过 Backend 全量门禁，状态为“已修复、待第二轮人工确认”，尚未合并 main，因此不标记 ✅。
...
| SEC-002 | 对象存储私有桶 | 🟠 | PR #7 已按私有桶 + 服务端 key + fail-closed 设计实现，且 READY 前验证真实图片文件头；待第二轮审查/部署 |
| SEC-003 | 临时签名下载 URL | 🟠 | PR #7 已实现短时 PUT/GET，GET 仅对 owner 的 READY final 对象签发；staging 仅在 READY commit 成功后清理，待第二轮审查 |
