...
test('Mini elder mode is profile-driven and Family role never enables it', () => {
  ...
  assert.match(home, /useState\(currentElderModeEnabled\)/)
  assert.match(home, /subscribeElderMode\(setElderMode\)/)
...
test('elder home only elevates existing production capabilities', () => {
  ...
  assert.match(home, /下面直接显示已经形成的足迹/)
  assert.match(home, /getTodayFootprint\(\)/)
